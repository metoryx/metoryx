...
class TestConv:
    def test_apply(self):
        ...
        input = jr.normal(jr.PRNGKey(1), shape=(4, 4, 4, 3))
        flax_output, variables = flax_conv.init_with_output(jr.PRNGKey(2), input)
        morphax_output, _ = apply(morphax_conv)(cast(Variables, variables), None, input)
    ...
    def test_apply_with_same_padding(self):
        ...
        input = jr.normal(jr.PRNGKey(1), shape=(2, 6, 6, 3))
        flax_output, variables = flax_conv.init_with_output(jr.PRNGKey(2), input)
        morphax_output, _ = apply(morphax_conv)(cast(Variables, variables), None, input)
        chex.assert_trees_all_close(morphax_output, flax_output)
    ...
    def test_apply_with_sequence_padding(self):
        ...
        input = jr.normal(jr.PRNGKey(1), shape=(2, 8, 8, 4))
        flax_output, variables = flax_conv.init_with_output(jr.PRNGKey(2), input)
        morphax_output, _ = apply(morphax_conv)(cast(Variables, variables), None, input)
        chex.assert_trees_all_close(morphax_output, flax_output)
    ...
    def test_apply_with_dilations(self):
        ...
        input = jr.normal(jr.PRNGKey(1), shape=(2, 10, 10, 3))
        flax_output, variables = flax_conv.init_with_output(jr.PRNGKey(2), input)
        morphax_output, _ = apply(morphax_conv)(cast(Variables, variables), None, input)
        chex.assert_trees_all_close(morphax_output, flax_output)
    ...
    def test_apply_with_groups(self):
        ...
        input = jr.normal(jr.PRNGKey(1), shape=(2, 5, 5, 6))
        flax_output, variables = flax_conv.init_with_output(jr.PRNGKey(2), input)
        morphax_output, _ = apply(morphax_conv)(cast(Variables, variables), None, input)
        chex.assert_trees_all_close(morphax_output, flax_output)
...
class TestDense:
    def test_apply(self):
        ...
        input = jr.normal(jr.PRNGKey(1), shape=(4, 2))
        flax_output, variables = flax_dense.init_with_output(jr.PRNGKey(2), input)
        morphax_output, _ = apply(morphax_dense)(cast(Variables, variables), None, input)
        chex.assert_trees_all_close(morphax_output, flax_output)
...
class TestEmbed:
    ...
    def test_apply(self):
        ...
        input = jr.randint(jr.PRNGKey(1), shape=(4,), minval=0, maxval=10)
        flax_output, variables = flax_embed.init_with_output(jr.PRNGKey(2), input)
        morphax_output, _ = apply(morphax_embed)(cast(Variables, variables), None, input)
        chex.assert_trees_all_close(morphax_output, flax_output)
    ...
    def test_attend(self):
        ...
        input = jr.normal(jr.PRNGKey(1), shape=(4, 5))
        input /= jnp.sum(input, axis=-1, keepdims=True)
        variables = init(morphax_embed)(jr.PRNGKey(2))

        flax_output = flax_apply_fn(variables, input)
        morphax_output, _ = apply(morphax_embed, lambda m: getattr(m, "attend"))(
            cast(Variables, variables), None, input
        )
        chex.assert_trees_all_close(morphax_output, flax_output)
