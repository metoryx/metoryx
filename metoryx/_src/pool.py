...
def pool(
    ...
    Args:
        input: Input array of shape (*batch_size, height, width, channels).
    ...
    num_batch_dims = input.ndim - len(kernel_size) - 1  # -1 for channel dimension
...
def avg_pool(
    ...
    output = pool(input, 0.0, lax.add, kernel_size, strides, padding)
...
def max_pool(
    ...
    output = pool(input, -jnp.inf, lax.max, kernel_size, strides, padding)
    return output
...
def min_pool(
    ...
    output = pool(input, jnp.inf, lax.min, kernel_size, strides, padding)
    return output
