...
class TestBatchNorm:
    def test_apply(self):
        ...
        input = jr.normal(jr.PRNGKey(1), shape=(4, 8))
        variables = flax_norm.init(jr.PRNGKey(2), input)

        flax_output, flax_updates = flax_norm.apply(variables, input, mutable=True)
        morphax_output, morphax_updates = apply(morphax_norm)(
            variables, None, input, is_training=True
        )
    ...
    def test_apply_with_notraining(self):
        ...
        input = jr.normal(jr.PRNGKey(1), shape=(4, 8))
        variables = flax_norm.init(jr.PRNGKey(2), input, use_running_average=False)

        # First update the running statistics
        _, variables = flax_norm.apply(variables, input, use_running_average=False, mutable=True)

        # Then test in eval mode
        flax_output = flax_norm.apply(variables, input, use_running_average=True)
        morphax_output, _ = apply(morphax_norm)(variables, None, input, is_training=False)
    ...
    def test_vectorized_apply(self):
        ...
        input = jr.normal(jr.PRNGKey(1), shape=(4, 4, 8))
        variables = jax.vmap(flax_norm.init)(jr.split(jr.PRNGKey(2), 4), input)

        flax_output, flax_updates = jax.vmap(
            lambda variables, input: flax_apply_fn(variables, input),
            axis_name="batch",
        )(variables, input)

        morphax_output, morphax_updates = jax.vmap(
            lambda variables, input: apply(morphax_norm)(variables, None, input, is_training=True),
            axis_name="batch",
        )(variables, input)
...
class TestLayerNorm:
    def test_apply(self):
        ...
        flax_output = flax_norm.apply(variables, input)
        morphax_output, _ = apply(morphax_norm)(variables, None, input)
